...
function search(dataset, fields, searchString) {
  var result = dataset.map(function (val) {
      ...
      return fields.map(function (val2) {
        if (Array.isArray(val[val2.name])) { //tags
          return val[val2.name].map(function (val3) {
            return { _id: val._id, target: val3, weight: val2.weight / 6 };
          });
        } else {
          return [{ _id: val._id, target: val[val2.name], weight: val2.weight }];
        }
      ...
      .map(function (val) { // split by space
        if (/\s/g.test(val.target)) {
          return val.target.split(/\s/g).map(function (val2, i) {
            ...
            return { _id: val._id, target: val2, weight: val.weight };
          });
        } else {
          return [{ _id: val._id, target: val.target, weight: val.weight }];
        }
      })
      .reduce(function (prev, curr) {
        return prev.concat(curr);
      ...
      .map(function (val) {
        return { _id: val._id, score: score(val, searchString) };
      })
      .sort(function (a, b) {
        if (a._id > b._id) {
          return 1;
      ...
      .reduce(function (prev, curr, i) {
        if (curr.score > 0 && prev.length > 0 && prev[prev.length-1]._id === curr._id) {
