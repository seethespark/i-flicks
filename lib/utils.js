...
function encode(statsD) {
    ...
    flicks.listUnencoded(0, encodeCount, {isSysAdmin: true}, function (err, docs) {
        if (err) {
            logger.errorNoReq('utils.encode', 'F02001', err, 2);
            return;
        }
        if (docs.data.length === 0) {
            setTimeout(encode, 2000, statsD);
        }
...
function sendUploadEmail(id) {
    ...
    flick.load(id, { isSysAdmin: true }, function (err, doc) {
        var user = new User({});
        user.load(doc.uploader, function (err, usr) {
            if (err) { logger.errorNoReq('utils.sendUploadEmail.loadUser', 'F020015', err, 2); }
            // setup e-mail data with unicode symbols
            var mailOptions = {
                from: global.iflicks_settings.mailFrom, // sender address
                to: usr.emailAddress, // list of receivers
                subject: 'i-flicks video', // Subject line
                //text: body, //, // plaintext body
                html: '<b>Your video, named "' + doc.name + '" has been encoded and is ready for viewing. ✔</b>' // html body
            };
...
function getFileDetails(sourcePath, callback) {
    ...
    ffmpeg.ffprobe(sourcePath, function (err, data) {
        ...
        retVal.rotatee = Math.max.apply(null, data.streams.map(function (probe) {
...
function cleanMedia() {
    ...
    flicks.listUnencoded(0, 1000,  {isSysAdmin: true}, function (err, docs) {
        if (err) {
